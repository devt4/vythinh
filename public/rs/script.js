...
$(document).on("click",".desktop-cart-wrapper > a", function(e){
	$.ajax({
		type: 'GET',
...
function convertToSlug(str) {
 ...
 function searchAutoComplete(){
	 var $input = $('.search-collection input');
	 $input.bind('keyup change paste propertychange', function() {
		 ...
		 if(key.length > 0 && key !== $(this).attr('data-history')){
			 ...
			 var str = '/search?q=filter=((title%3Aproduct**' + key + '))&view=autocomplate';
			 $.ajax({
				 url: str,
				 type: 'GET',
				 async: true,
	 ...
	 $('body').click(function(evt) {
		 ...
		 if (target.id !== 'search-results' && target.id !== 'pos_search_top') {
			 $("#search-results").hide();
		 }
	 });
	 $('body').on('click', '.search-collection input', function() {
		 if ($(this).is(":focus")) {
			 if ($(this).val() != '') {
				 $("#search-results").show();
			 }
	  ...
	  $('body').on('click', '.variant-image-loop', function(e) {
		 console.log(1);
		 e.preventDefault();
		 var img = $(this).attr("data-src");

		 var mau = $(this).attr('titlexx');
		 var hihi = [];
		 var final = mau.substr(0,1).toUpperCase()+mau.substr(1);



		 $('.product-item').removeClass('ok');

		 $(this).parents('.product-item').addClass('ok');
		 /*
		$('.ok .hiddenxx[alt='+final+']').each( (i, value) => {
			 hihi.push(value);
console.log($(this).attr('src'));
		 ...
		 $('.product-item.ok .hiddenxx[alt='+final+']').each(function() {
			 hihi.push($(this).attr('src2'));
...
console.log(hihi);
		 ...
		 //var final = hihi[1].split('src2"');


		 /*
		 $('.hiddenxx[alt='+final+']').each(function(){
			 hihi.push($(this).attr('src2'));
		 });
*/
		 // var imgxx = $(this).parents(".product-item").find('.hiddenxx');





		 $(this).parents(".product-item").find(".imghover1").attr("src",img);
		 if(hihi[1] != ''){
			 $(this).parents(".product-item").find(".imghover2").attr("src",img);
